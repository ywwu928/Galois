...
void TypeAssignment(Graph& graph) {
    ...
    galois::on_each([&](unsigned tid, unsigned nthreads) {
        ...
        for (uint32_t lid=start; lid<end; lid++) {
            auto& master_data = graph.getData(lid);
    ...
    galois::on_each([&](unsigned tid, unsigned nthreads) {
        ...
        for (uint32_t index=start; index<end; index++) {
            ...
            auto& mirror_data = graph.getData(lid);
    ...
    galois::on_each([&](unsigned tid, unsigned nthreads) {
        ...
        for (uint64_t index=start; index<end; index++) {
            auto& edge_data = graph.getEdgeDataDirect(index);
...
/*
void PropertyConstruction (typedef Graph& graph) {
    // sanity check
    uint64_t sum = 0;
    for (uint32_t i=0; i<8; i++) {
