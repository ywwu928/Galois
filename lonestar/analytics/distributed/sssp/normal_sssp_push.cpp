...
struct FirstItr_SSSP {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
#ifdef GALOIS_ENABLE_GPU
      std::string impl_str(syncSubstrate->get_run_identifier("SSSP"));
      galois::StatTimer StatTimer_cuda(impl_str.c_str(), REGION_NAME);
      StatTimer_cuda.start();
    ...
    syncSubstrate->sync<writeDestination, readSource, Reduce_min_dist_current, Bitset_dist_current, async>("SSSP");

    galois::runtime::reportStat_Tsum(
        "SSSP", syncSubstrate->get_run_identifier("NumWorkItems"),
        __end - __begin);
  }
