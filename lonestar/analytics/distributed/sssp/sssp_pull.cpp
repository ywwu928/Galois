  ...
  void static go(Graph& _graph, 
    ...
    do {
      ...
      if (personality == GPU_CUDA) {
        ...
        galois::do_all(galois::iterate(nodesWithEdges), SSSP(&_graph, dga, Access_total, Mirror_access_total, Mirror_access_write),
...
int main(int argc, char** argv) {
  ...
  for (auto run = 0; run < numRuns; ++run) {
    ...
    if (execution == Async) {
      SSSP<true>::go(*hg, Access_total, Mirror_access_total, Mirror_access_write);
    } else {
      SSSP<false>::go(*hg, Access_total, Mirror_access_total, Mirror_access_write);
    }
    StatTimer_main.stop();
