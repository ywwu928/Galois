...
for i in $process
do
    srun -N 1 -n ${i} $program "$graph/data_10.gr" -graphTranspose="$graph/data_10.tgr" -startNode=44 -t=4 --runs=1 -partition=iec --exec=Sync -statFile="$result/data_10_${i}procs_stat" > "$result/data_10_${i}procs"
done
