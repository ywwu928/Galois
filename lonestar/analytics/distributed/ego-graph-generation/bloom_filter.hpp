...
public:
     ...
     desired_false_positive_probability_(p.false_positive_probability)
   {
      ...
      counter_table_.resize(table_size_, 0);
   }
   ...
   inline void set_parameters(bloom_parameters& p)
   {
      projected_element_count_ = p.projected_element_count;
      inserted_element_count_ = 0;
      random_seed_ = (p.random_seed * 0xA5A5A5A5) + 1;
   ...
   inline bool operator == (const bloom_filter& f) const
   {
      if (this != &f)
      {
         return
            ...
            (counter_table_.size()                   == f.counter_table_.size()                  ) &&
            ...
            (salt_                               == f.salt_                              ) &&
            (counter_table_                          == f.counter_table_                         ) ;
      }
      else
         return true;
   }
   ...
   inline bool insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      {
         compute_indices(hash_ap(key_begin, length, salt_[i]), counter_index);

         counter_table_[counter_index] += 1;

         if (counter_table_[counter_index] < threshold_) {
             exceed = false;
         }
      }
   ...
   inline bool insert(const T& t)
   {
      ...
      bool exceed = insert(reinterpret_cast<const unsigned char*>(&t),sizeof(T));
      return exceed;
   }

   inline bool insert(const std::string& key)
   {
      bool exceed = insert(reinterpret_cast<const unsigned char*>(key.data()),key.size());
      return exceed;
   }

   inline bool insert(const char* data, const std::size_t& length)
   {
      bool exceed = insert(reinterpret_cast<const unsigned char*>(data),length);
      return exceed;
   }
...
protected:
   ...
   inline virtual void compute_indices(const bloom_type& hash, std::size_t& counter_index) const
   {
      counter_index = hash % table_size_;
   }
   ...
   std::vector<unsigned int> counter_table_;
...
/*
  ...
  counter_table_[bit_index >> n] |= bit_mask[bit_index & (bits_per_char - 1)];
