...
struct FormVector {
  ...
  std::vector<std::vector<uint64_t>>& info;

  FormVector(Graph* _graph, std::vector<std::vector<uint64_t>>& _info) : graph(_graph), info(_info) {}

  void static go(Graph& _graph, std::vector<std::vector<uint64_t>>& info) {
    auto mirrorSize = _graph.numMirrors();
    info.reserve(mirrorSize);
    ...
    if (personality == GPU_CUDA) {
      ...
      galois::do_all(
          ...
          FormVector{&_graph, info}, galois::no_stats(),
          galois::loopname(
              syncSubstrate->get_run_identifier("FormVector").c_str()));
    }
  }
  ...
  void operator()(GNode src) const {
      ...
      std::vector<uint64_t> temp {snode.owner_host, snode.read, snode.write, snode.read+snode.write}; // order: {owner, read, write, access}
...
void countAccess (std::vector<std::vector<uint64_t>>& info,
    ...
    // sort the info vector first
    std::sort(info.begin(), info.end(), sortAccess);

    //mirror_read[0] += 1;
    //mirror_write[0] += 1;
    //remote_read[0] += 1;
    //remote_write[0] += 1;
    //remote_read_to_host[0][0] += 1;
    //remote_write_to_host[0][0] += 1;
    //remote_comm_to_host[0][0] += 1;
    //info.clear();
	...
	for (int i=0; i<10; i++) {
		for (int j=0; j<chunk_size; j++) {
			...
			uint64_t owner_host = info[index][0];
			uint64_t read = info[index][1];
			uint64_t write = info[index][2];
...
struct FirstItr_BFS {
  ...
  FirstItr_BFS(Graph* _graph,
              std::vector<std::vector<uint64_t>>& _info,
