...
struct BFSRemote {
  ...
  BFSRemote(Graph* _graph, galois::DynamicBitSet* _active_bitset_ptr)
      : graph(_graph),
  ...
  void static go(Graph& _graph, galois::DynamicBitSet* _active_bitset_ptr) {
      ...
      galois::do_all(
          galois::iterate(remoteNodes), BFSRemote{&_graph, _active_bitset_ptr},
...
/*
    ...
    for (auto jj : graph->inEdges(dst)) {
        ...
        if (snode.dist_current + 1 < ddist) {
            ...
            if (bitset_dist_current_odd.test(src)) {
                send = true;
            }
            else {
                send = false;
...
struct BFSMaster {
  ...
  BFSMaster(Graph* _graph, galois::DynamicBitSet* _active_bitset_ptr, galois::DynamicBitSet* _dirty_bitset_ptr)
      : graph(_graph),
        active_bitset_ptr(_active_bitset_ptr),
        dirty_bitset_ptr(_dirty_bitset_ptr) {}
  ...
  void static go(Graph& _graph, galois::DynamicBitSet* _active_bitset_ptr, galois::DynamicBitSet* _dirty_bitset_ptr) {
      ...
      galois::do_all(
          galois::iterate(masterNodes), BFSMaster{&_graph, _active_bitset_ptr, _dirty_bitset_ptr},
          galois::steal(), galois::no_stats());
  }
...
constexpr static const char* const name = "Distributed Breadth-First Search (Pull)";
constexpr static const char* const desc = "Distributed Breadth-First Search (Pull)";
