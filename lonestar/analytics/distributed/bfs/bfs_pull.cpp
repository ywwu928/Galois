...
struct BFS {
  ...
  void static go(Graph& _graph, 
    ...
    // std::cout << "Graph size = " << _graph.size() << std::endl;
    // std::cout << "Bitset size = " << bitset_dist_current.size() << std::endl;
    // std::cout << "Number of master nodes = " << _graph.numMasters() << std::endl;
    // const auto mirrorRange = _graph.getMirrorRanges();
    // for (long unsigned int i=0; i<mirrorRange.size(); i++) {
      // std::cout << "Mirror Range = (" << mirrorRange[i].first << ", " << mirrorRange[i].second << ")" << std::endl;
    // }
    ...
    // galois::runtime::reportStat_Single(REGION_NAME, "TotalAccess", Access_total.reduce());
  ...
  void operator()(GNode src) const {
    ...
    /*
    if (galois::runtime::getSystemNetworkInterface().ID == 0) {
        auto masterID = graph->getHostID(src);
        if (masterID == 0) {
            std::cout << "master ";
        }
        else {
        ...
        std::cout << "src, local ID = " << src << ", global ID = " << graph->getGID(src) << std::endl;
        std::cout << std::endl;
        std::cout << std::endl;
        std::cout << std::endl;
    }
    */
    ...
    // }
    // auto masterID = graph->getHostID(src);
    // if (masterID != galois::runtime::getSystemNetworkInterface().ID) {
        // std::cout << "src Master ID = " << masterID << std::endl;
        // std::cout << "src Network ID = " << galois::runtime::getSystemNetworkInterface().ID << std::endl;
        // for (auto jj : graph->edges(src)) {
            // GNode dst         = graph->getEdgeDst(jj);
            // auto& dnode       = graph->getData(dst);
            // std::cout << "dst Master ID = " << graph->getHostID(dst) << std::endl;
            // std::cout << "dst dist = " << dnode.dist_current << std::endl;
    ...
    for (auto jj : graph->edges(src)) {
        ...
        /*
        if (galois::runtime::getSystemNetworkInterface().ID == 0) {
            auto masterID = graph->getHostID(dst);
            if (masterID == 0) {
                std::cout << "master ";
            }
            else {
            ...
            std::cout << "dst, local ID = " << dst << ", global ID = " << graph->getGID(dst) << std::endl;
        ...
        */
        ...
        /*
        if (galois::runtime::getSystemNetworkInterface().ID == 0) {
            std::cout << "new dist = " << new_dist << ", old dist = " << old_dist << std::endl;
            if (old_dist > new_dist) {
                auto masterID = graph->getHostID(src);
                if (masterID == 0) {
                    std::cout << "Write to master node" << std::endl;
                }
                else {
                    std::cout << "Write to mirror node" << std::endl;
                }
            }
            else {
