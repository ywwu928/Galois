...
struct InitializeGraph2 {
  ...
  InitializeGraph2(Graph* _graph) : graph(_graph), net(galois::runtime::getSystemNetworkInterface()) {}
  ...
  void static go(Graph& _graph) {
    ...
    std::function<void(void)> func = [&]() {
            syncSubstrate->poll_for_remote_work_dedicated<Reduce_add_current_degree>();
    };
    ...
    syncSubstrate->sync_update_buf<Reduce_add_current_degree>(0);
#endif
...
struct KCoreStep1 {
  ...
  KCoreStep1(cll::opt<uint32_t>& _kcore, Graph* _graph, DGTerminatorDetector& _dga)
      : local_k_core_num(_kcore), graph(_graph), active_vertices(_dga), net(galois::runtime::getSystemNetworkInterface()) {}
  ...
  void static go(Graph& _graph) {
    ...
    do {
      ...
      std::string reset_buf_str("ResetBuf_Round_" + std::to_string(iterations));
      ...
      std::string sync_buf_str("SyncBuf_Round_" + std::to_string(iterations));
      ...
      // dedicate a thread to poll for remote messages
      std::function<void(void)> func = [&]() {
              syncSubstrate->poll_for_remote_work_dedicated<Reduce_add_trim>();
      };
      galois::substrate::getThreadPool().runDedicated(func);
#endif
      ...
      syncSubstrate->sync_update_buf<Reduce_add_trim>(0);
    ...
    } while ((iterations < maxIterations) && dga.reduce(syncSubstrate->get_run_identifier()));
  }
