...
struct KCorePullRemote {
  Graph* graph;

  galois::runtime::NetworkInterface& net;

  KCorePullRemote(Graph* _graph) : graph(_graph), net(galois::runtime::getSystemNetworkInterface()) {}

  void static go(Graph& _graph) {
    const auto& remoteNodes = _graph.remoteNodesRange();

    galois::do_all(galois::iterate(remoteNodes), KCorePullRemote{&_graph},
...
struct KCorePullMaster {
  Graph* graph;

  KCorePullMaster(Graph* _graph) : graph(_graph) {}

  void static go(Graph& _graph) {
    const auto& masterNodes = _graph.masterNodesRange();

    galois::do_all(galois::iterate(masterNodes), KCorePullMaster{&_graph},
                   galois::steal(), galois::no_stats());
  }
  ...
  void static go(Graph& _graph) {
    ...
    do {
      ...
      if (pull) {
          ...
          KCorePullRemote::go(_graph);
          _net.flushRemoteWork();
          KCorePullMaster::go(_graph);
...
constexpr static const char* const name = "Distributed KCore Extraction (Hybrid)";
constexpr static const char* const desc = "Distributed KCore Extraction (Hybrid)";
