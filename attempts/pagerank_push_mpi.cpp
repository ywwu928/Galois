...
struct PageRank {
  ...
  void static go(Graph& _graph, 
    ...
    do {
      ...
      round_stream << "#####   Round " << _num_iterations << "   #####" << std::endl;
      round_stream << "host " << host_id << " round master accesses: " << master_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round master writes: " << master_write_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round mirror accesses: " << mirror_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round mirror writes: " << mirror_write_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round local accesses: " << local_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round local writes: " << local_write_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round remote accesses: " << remote_round.read_local() << std::endl;
      round_stream << "host " << host_id << " round remote writes: " << remote_write_round.read_local() << std::endl;
      ...
      round_stream << "host " << host_id << " number of dirty mirrors: " << dirty_count << std::endl;
      round_stream << "host " << host_id << " number of touched mirrors: " << touched_count << std::endl;
      ...
      round_stream << "host " << host_id << " number of cached mirrors: " << cached_count << std::endl;
      round_stream << "host " << host_id << " number of caches with count 1: " << cached_one_count << std::endl;
      round_stream << "host " << host_id << " number of caches with count 2: " << cached_two_count << std::endl;
      
      for (uint32_t i=0; i<num_hosts; i++) {
          round_stream << "host " << host_id << " mirror access to host " << i << ": " << mirror_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " mirror write to host " << i << ": " << mirror_write_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " dirty mirrors for host " << i << ": " << num_mirror_write_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " touched mirrors for host " << i << ": " << num_mirror_touched_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " remote access to host " << i << ": " << remote_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " remote write to host " << i << ": " << remote_write_to_host[i].read_local() << std::endl;
          round_stream << "host " << host_id << " cached mirrors from host " << i << ": " << cached_from_host[i] << std::endl;
      }

      // galois::gPrint(round_stream.str());
      if (host_id == 0) {
          for (uint32_t x=1; x<num_hosts; x++) {
              std::string message;
              syncSubstrate->mpi_recv(message, x);
              galois::gPrint(message);
          }
      }
      else {
          syncSubstrate->mpi_send(round_stream.str(), 0);
      }
...
*/
    ...
    run_stream << "#####   Summary   #####" << std::endl;
    run_stream << "host " << host_id << " total master accesses: " << master_total.read_local() << std::endl;
    run_stream << "host " << host_id << " total master writes: " << master_write_total.read_local() << std::endl;
    run_stream << "host " << host_id << " total mirror accesses: " << mirror_total.read_local() << std::endl;
    run_stream << "host " << host_id << " total mirror writes: " << mirror_write_total.read_local() << std::endl;
    galois::gPrint(run_stream.str());
  }
...
int main(int argc, char** argv) {
  ...
  galois::DGAccumulator<uint64_t> remote_round;
  galois::DGAccumulator<uint64_t> remote_write_round;
  galois::DGAccumulator<uint64_t> remote_to_host[num_host];
  galois::DGAccumulator<uint64_t> remote_write_to_host[num_host];
  ...
  uint64_t host_id = galois::runtime::getSystemNetworkInterface().ID;
  stat_stream << "#####   Stat   #####" << std::endl;
  stat_stream << "host " << host_id << " total edges: " << hg->sizeEdges() << std::endl;
  galois::gPrint(stat_stream.str());

  for (auto run = 0; run < numRuns; ++run) {
    std::ostringstream run_stream;
    run_stream << "#####   Run " << run << "   #####" << std::endl;
    galois::gPrint(run_stream.str());
